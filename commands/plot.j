define-class LineGraph-Sub-FlameGraph-View-Input-Handler
    'on-key :
        fn (&self &view &key)
            match &key
                "q"
                ...
                foreach widget-name (&view 'widgets)
                    ...
                    match response
                        ...
                        'range-selected
                            ...
                            foreach linegraph-widget-name (&view 'widgets)
                            ...
                            views <-
                                "flamegraph" :
                                    (View 'new) rows cols
                                        'name          : "Flame Graph"
                                        'input-handler : (new-instance LineGraph-Sub-FlameGraph-View-Input-Handler)
...
plot-command =
    fn (&profile)
        &current-view @ ('set-input-handler (new-instance Plot-View-Input-Handler))
        ...
        foreach event (&profile 'event-count-by-type)
            ...
            &current-view @
                'add-widget name
                    (Line-Graph 'new) &profile event offset 0
